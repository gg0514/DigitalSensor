...
public class SubActivityCaliChTemperatureElectrode extends Fragment {
    private static SubActivityCaliChTemperatureElectrode.ChTemperatureElectrodeSendThread m_ChTemperatureElectrodeSendThread;
    private static SubActivityCaliChTemperatureElectrode.UpdateCurrentValueThread m_UpdateCurrentValueThreadThread;
    EditText edCurrent;
    Button btApply;
    TextView tvHelpText;
    ImageView ivIcon;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.sub_activity_cali_ch_temperatureelectrode, container, false);

        ivIcon = (ImageView)v.findViewById(R.id.Cali_Ch_TE_Help_Icon_Beaker);
        pgCircle = (ProgressBar)v.findViewById(R.id.Cali_Ch_TE_Help_Progress);
        tvHelpText = (TextView)v.findViewById(R.id.Cali_Ch_TE_Help_Text1);
        edCurrent = (EditText)v.findViewById(R.id.Cali_Ch_TE_Current_Ed);
        btApply = (Button)v.findViewById(R.id.Cali_Ch_TE_Option2_Apply);

        edCalibration = (EditText)v.findViewById(R.id.Cali_Ch_TE_Calibration_Ed);
        ...
        v.findViewById(R.id.Cali_Ch_TE_Main).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ch_TE_UpDown_Up).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ch_TE_UpDown_Down).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ch_TE_Option2_Stop).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ch_TE_Option2_Apply).setOnClickListener(mClickListener);
        ...
        m_ChTemperatureElectrodeSendThread = new ChTemperatureElectrodeSendThread(true);
    ...
    @Override
    public void onStop() {
        if (m_ChTemperatureElectrodeSendThread.isAlive()){
            m_ChTemperatureElectrodeSendThread.stopChTemperatureElectrodeSendThread();
        }
    ...
    public static SubActivityCaliChTemperatureElectrode newInstance(String text) {

        SubActivityCaliChTemperatureElectrode f = new SubActivityCaliChTemperatureElectrode();
        Bundle b = new Bundle();
    ...
    public void startChTemperatureElectrodeSendThread(){
        if (!m_ChTemperatureElectrodeSendThread.isAlive()){
            m_ChTemperatureElectrodeSendThread = new ChTemperatureElectrodeSendThread(true);
            m_ChTemperatureElectrodeSendThread.start();
        }
    }
    ...
    Button.OnClickListener mClickListener  = new View.OnClickListener() {
        public void onClick(View v) {
            ...
            DecimalFormat df2Figure = new DecimalFormat("0.00");
            switch (v.getId()) {
                case R.id.Cali_Ch_TE_UpDown_Up:
                    ...
                    break;
                case R.id.Cali_Ch_TE_UpDown_Down:
                    dbCalibration = dbCalibration - 0.01;
                    dbCalibration = Double.parseDouble(df2Figure.format(dbCalibration));
                    edCalibration.setText(Double.toString(dbCalibration));
                    break;
                case R.id.Cali_Ch_TE_Option2_Stop :
                    m_nCaliStatus = 16;
                    FileLogWriter(getString(R.string.multi_cal_ch_temp_ele),"Click Stop");
                    break;
                case R.id.Cali_Ch_TE_Option2_Apply:
                    m_DataCommunicationThread.stopDCThread();
//                    DecimalFormat df = new DecimalFormat("##.00 ");
//                   // String strTemp = edCalibration.getText().toString()+" ";
//                    Double dbTemp = 0.0;
//                    dbTemp = Double.parseDouble(edCalibration.getText().toString());
//                    String strPayload1 = df.format(dbTemp);
                    ...
                    m_nCaliStatus = 1;
                    startChTemperatureElectrodeSendThread();
                    FileLogWriter(getString(R.string.multi_cal_ch_temp_ele),"Click Apply");
    ...
    class ChTemperatureElectrodeSendThread extends Thread {
        private boolean isPlay = false;
        private int nCount = 0;
        public ChTemperatureElectrodeSendThread(boolean isPlay) {this.isPlay = isPlay;}
        public void stopChTemperatureElectrodeSendThread(){
            isPlay = !isPlay;
            nCount = 0;
        ...
        public void run() {
            ...
            while (isPlay) {
                try {
                    ...
                    if (m_nCaliStatus != 1)
                    {
                        stopChTemperatureElectrodeSendThread();
                    }
                    if (nCount > 300)
                    {
                        m_nCaliStatus = 15;
