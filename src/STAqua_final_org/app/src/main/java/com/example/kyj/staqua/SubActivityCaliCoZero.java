...
public class SubActivityCaliCoZero extends Fragment {
    private static SubActivityCaliCoZero.CoZeroSendThread m_CoZeroSendThread;
    private static SubActivityCaliCoZero.UpdateCurrentValueThread m_UpdateCurrentValueThreadThread;
    EditText edCurrent;
    Button btApply;
    TextView tvHelpText;
    ImageView ivIcon;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.sub_activity_cali_co_zero, container, false);
        ivIcon = (ImageView)v.findViewById(R.id.Cali_Co_Zero_Help_Icon_Beaker);
        pgCircle = (ProgressBar)v.findViewById(R.id.Cali_Co_Zero_Help_Progress);
        tvHelpText = (TextView)v.findViewById(R.id.Cali_Co_Zero_Help_Text1);
        edCurrent = (EditText)v.findViewById(R.id.Cali_Co_Zero_Current_Ed);
        btApply = (Button)v.findViewById(R.id.Cali_Co_Zero_Option2_Apply);
        v.findViewById(R.id.Cali_Co_Zero_Option2_Stop).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Co_Zero_Option2_Apply).setOnClickListener(mClickListener);

        DecimalFormat df2Figure = new DecimalFormat("0.00");
        edCurrent.setText( df2Figure.format(m_dCo));

        m_CoZeroSendThread = new CoZeroSendThread(true);
        m_UpdateCurrentValueThreadThread = new UpdateCurrentValueThread(true);
        m_UpdateCurrentValueThreadThread.start();
        return v;
    }

    public static SubActivityCaliCoZero newInstance(String text) {
        SubActivityCaliCoZero f = new SubActivityCaliCoZero();
        Bundle b = new Bundle();
    ...
    @Override
    public void onStop() {
        if (m_CoZeroSendThread != null){
            m_CoZeroSendThread.stopCoZeroSendThread();
        }
        super.onStop();
    }


    public void startCoZeroSendThread(){
        if (!m_CoZeroSendThread.isAlive()){
            m_CoZeroSendThread = new CoZeroSendThread(true);
            m_CoZeroSendThread.start();
        }
    ...
    Button.OnClickListener mClickListener  = new View.OnClickListener() {
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.Cali_Co_Zero_Option2_Stop :
                    m_nCaliStatus = 16;
                    FileLogWriter(getString(R.string.multi_cal_co_zero),"Click Stop");
                    break;
                case R.id.Cali_Co_Zero_Option2_Apply:
                    ...
                    m_nCaliStatus = 1;
                    startCoZeroSendThread();
                    btApply.setEnabled(FALSE);
                    DisplayHelpText(m_nCaliStatus);
                    FileLogWriter(getString(R.string.multi_cal_co_zero),"Click Apply");
                    break ;
            }
        }
    };

    class CoZeroSendThread extends Thread {
        private boolean isPlay = false;
        private int nCount = 0;
        public CoZeroSendThread(boolean isPlay) {this.isPlay = isPlay;}
        public void stopCoZeroSendThread(){
            isPlay = !isPlay;
            nCount = 0;
