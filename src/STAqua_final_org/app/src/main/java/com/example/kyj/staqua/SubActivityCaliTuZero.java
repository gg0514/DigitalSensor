...
public class SubActivityCaliTuZero extends Fragment {
    private static SubActivityCaliTuZero.TuZeroSendThread m_TuZeroSendThread;
    private static SubActivityCaliTuZero.UpdateCurrentValueThread m_UpdateCurrentValueThreadThread;
    EditText edCurrent;
    Button btApply;
    TextView tvHelpText;
    ImageView ivIcon;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.sub_activity_cali_tu_zero, container, false);

        ivIcon = (ImageView)v.findViewById(R.id.Cali_Tu_Zero_Help_Icon_Beaker);
        pgCircle = (ProgressBar)v.findViewById(R.id.Cali_Tu_Zero_Help_Progress);
        tvHelpText = (TextView)v.findViewById(R.id.Cali_Tu_Zero_Help_Text1);
        edCurrent = (EditText)v.findViewById(R.id.Cali_Tu_Zero_Current_Ed);
        btApply = (Button)v.findViewById(R.id.Cali_Tu_Zero_Option2_Apply);
        v.findViewById(R.id.Cali_Tu_Zero_Option2_Stop).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Tu_Zero_Option2_Apply).setOnClickListener(mClickListener);

        DecimalFormat df2Figure = new DecimalFormat("0.00");
        edCurrent.setText( df2Figure.format(m_dTu));

        m_TuZeroSendThread = new TuZeroSendThread(true);
        m_UpdateCurrentValueThreadThread = new UpdateCurrentValueThread(true);
        m_UpdateCurrentValueThreadThread.start();
        return v;
    }

    public static SubActivityCaliTuZero newInstance(String text) {
        SubActivityCaliTuZero f = new SubActivityCaliTuZero();
        Bundle b = new Bundle();
    ...
    @Override
    public void onStop() {
        if (m_TuZeroSendThread != null){
            m_TuZeroSendThread.stopTuZeroSendThread();
        }
        super.onStop();
    }

    public void startTuZeroSendThread(){
        if (!m_TuZeroSendThread.isAlive()){
            m_TuZeroSendThread = new TuZeroSendThread(true);
            m_TuZeroSendThread.start();
        }
    ...
    Button.OnClickListener mClickListener  = new View.OnClickListener() {
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.Cali_Tu_Zero_Option2_Stop :
                    m_nCaliStatus = 16;
                    FileLogWriter(getString(R.string.multi_cal_tu_zero),"Click Stop");
                    break;
                case R.id.Cali_Tu_Zero_Option2_Apply:
                    ...
                    m_nCaliStatus = 1;
                    startTuZeroSendThread();
                    btApply.setEnabled(FALSE);
                    DisplayHelpText(m_nCaliStatus);
                    FileLogWriter(getString(R.string.multi_cal_tu_zero),"Click Apply");
                    break ;
            }
        }
    };

    class TuZeroSendThread extends Thread {
        private boolean isPlay = false;
        private int nCount = 0;
        public TuZeroSendThread(boolean isPlay) {this.isPlay = isPlay;}
        public void stopTuZeroSendThread(){
            isPlay = !isPlay;
            nCount = 0;
