...
public class SubActivityCaliPhTemperatureElectrode extends Fragment {

    private static SubActivityCaliPhTemperatureElectrode.PhTemperatureElectrodeSendThread m_PhTemperatureElectrodeSendThread;
    private static SubActivityCaliPhTemperatureElectrode.UpdateCurrentValueThread m_UpdateCurrentValueThreadThread;
    EditText edCurrent;
    Button btApply;
    TextView tvHelpText;
    ImageView ivIcon;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.sub_activity_cali_ph_temperatureelectrode, container, false);


        ivIcon = (ImageView)v.findViewById(R.id.Cali_Ph_TE_Help_Icon_Beaker);
        pgCircle = (ProgressBar)v.findViewById(R.id.Cali_Ph_TE_Help_Progress);
        tvHelpText = (TextView)v.findViewById(R.id.Cali_Ph_TE_Help_Text1);
        edCurrent = (EditText)v.findViewById(R.id.Cali_Ph_TE_Current_Ed);
        btApply = (Button)v.findViewById(R.id.Cali_Ph_TE_Option2_Apply);

        edCalibration = (EditText)v.findViewById(R.id.Cali_Ph_TE_Calibration_Ed);
        ...
        v.findViewById(R.id.Cali_Ph_TE_Main).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ph_TE_UpDown_Up).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ph_TE_UpDown_Down).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ph_TE_Option2_Stop).setOnClickListener(mClickListener);
        v.findViewById(R.id.Cali_Ph_TE_Option2_Apply).setOnClickListener(mClickListener);
        ...
        m_PhTemperatureElectrodeSendThread = new PhTemperatureElectrodeSendThread(true);
    ...
    @Override
    public void onStop() {
        if (m_PhTemperatureElectrodeSendThread.isAlive()){
            m_PhTemperatureElectrodeSendThread.stopPhTemperatureElectrodeSendThread();
        }
    ...
    public static SubActivityCaliPhTemperatureElectrode newInstance(String text) {
        SubActivityCaliPhTemperatureElectrode f = new SubActivityCaliPhTemperatureElectrode();
        Bundle b = new Bundle();
    ...
    public void startPhTemperatureElectrodeSendThread(){
        if (!m_PhTemperatureElectrodeSendThread.isAlive()){
            m_PhTemperatureElectrodeSendThread = new PhTemperatureElectrodeSendThread(true);
            m_PhTemperatureElectrodeSendThread.start();
        }
    }
    ...
    Button.OnClickListener mClickListener  = new View.OnClickListener() {
        public void onClick(View v) {
            ...
            DecimalFormat df2Figure = new DecimalFormat("0.00");
            switch (v.getId()) {
                case R.id.Cali_Ph_TE_UpDown_Up:
                    ...
                    break;
                case R.id.Cali_Ph_TE_UpDown_Down:
                    dbCalibration = dbCalibration - 0.01;
                    dbCalibration = Double.parseDouble(df2Figure.format(dbCalibration));
                    edCalibration.setText(Double.toString(dbCalibration));
                    break;
                case R.id.Cali_Ph_TE_Option2_Stop :
                    m_nCaliStatus = 16;
                    FileLogWriter(getString(R.string.multi_cal_ph_temp_ele),"Click Stop");
                    break;
                case R.id.Cali_Ph_TE_Option2_Apply:
           ...
           //         String strPayload1 = edCalibration.getText().toString()+" ";
                    ...
                    m_nCaliStatus = 1;
                    startPhTemperatureElectrodeSendThread();
                    FileLogWriter(getString(R.string.multi_cal_ph_temp_ele),"Click Apply");
    ...
    class PhTemperatureElectrodeSendThread extends Thread {
        private boolean isPlay = false;
        private int nCount = 0;
        public PhTemperatureElectrodeSendThread(boolean isPlay) {this.isPlay = isPlay;}
        public void stopPhTemperatureElectrodeSendThread(){
            isPlay = !isPlay;
            nCount = 0;
        ...
        public void run() {
            ...
            while (isPlay) {
             ...
             //       SendProtocol((byte)0x67, 1, "1", m_strMode);
                    if (m_nCaliStatus != 1)
                    {
                        stopPhTemperatureElectrodeSendThread();
                    }
                    if (nCount > 300)
                    {
                        m_nCaliStatus = 15;
