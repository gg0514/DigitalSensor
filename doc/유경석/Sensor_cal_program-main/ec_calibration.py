...
class ECCalibrationModule:
    def __init__(self, main_window, serial_comm):
        try:
            ...
            self.main.show_error_message(f"EC Calibration Module 초기화 오류: {e}")
    ...
    def calibration_step2(self):  # Sample Calibration
        try:
            if self.current_step == 2:
                ...
                self._active = True
                self.waiting_for_ok = True
                self.main.text_browser.append("EC Zero Calibration value : 1,413 uS/cm")
                self.serial_comm.send_data("CALS1PS 1413\r")
            else:
    ...
    def start_monitoring_step1(self):
        try:
            ...
            self.main.text_browser.append("EC Zero Calibration applied, starting monitoring...")
    ...
    def start_monitoring_step2(self):
        try:
            self.ui_controller.enable_calibration_buttons(False)    # 버튼 비활성화
            self.main.text_browser.append("EC Sample Calibration applied, starting monitoring...")
            self.monitoring_active = True
            self.timer.start(5000)
        except Exception as e:
    ...
    def process_status(self, status):
        try:
            ...
            if self.current_step == 1:
                ...
                elif status[0] != '04' or status[1] != '08' or (status[2] != '01' and status[2] != '02'):
                    self.main.text_browser.append("Error in calibration STEP 1")
                    self.reset_to_step1()
                elif status == ['04', '08', '02']:
                    self.complete_step1()
            
            elif self.current_step == 2:
                if status == ['04', '04', '01']:
                    self.main.text_browser.append("Processing... waiting in 5 seconds")
                elif status[0] != '04' or status[1] != '04' or (status[2] != '01' and status[2] != '02'):
                    self.main.text_browser.append("Error in calibration STEP 2")
                    self.reset_to_step1()    
                elif status == ['04', '04', '02']:
        ...
        try:
            ...
            self.main.text_browser.setHtml(""" 
                            <div style="line-height: 1.5;">
                                ...
                                (Immerse the EC sensor in the conical tube containing EC Sample and wait for stabilization 
                                for about 5 minutes., After 5 minutes, Press the "1 Point Sample" button.)
                            </div>          """)
            self.monitoring_active = False
            #self._active = True
        ...
        try:
            ...
            self.main.text_browser.setHtml(""" 
                            <div style="line-height: 1.5;">
                                ...
                                (Expose the sensor electrodes to the air without touching anything, 
                                let the sensor dry completely, and Press the "zero calibration" button 
                                (until the measurement value stabilizes).)
                            </div>          """)
        except Exception as e:
